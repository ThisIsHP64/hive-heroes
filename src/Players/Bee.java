...
public class Bee extends Player {
    ...
    public int drainAllNectar() {
        ...
        if (carried > 0) BeeStats.setNectar(0);
        return carried;
    }

    public void applyDamage(int amount) {
        if (BeeStats.isDead()) return;
    ...
    public void obtainTunic() {
        if (BeeStats.hasTunic()) return; // prevent icon duplication
    ...
    public void obtainBlueTunic() {
        if (BeeStats.hasBlueTunic()) return; // prevent duplicate icons
    ...
    public void spawnFlower() {
        ...
        if (TeleportManager.getCurrentGameState() == GameState.GRASSLEVEL) {
            switch (flowerNumber) {
                case 1:
                    RareSunflowerwithFlowers rareSunflower =
                        new RareSunflowerwithFlowers(4, this.map.getMapTile(randomX, randomY).getLocation());
                    if (distance - lastMilestone >= 100 &&
                        this.map.getMapTile(randomX, randomY).getTileType() == TileType.PASSABLE) {
                ...
                case 2:
                    BlueBorah blueBorah =
                        new BlueBorah(4, this.map.getMapTile(randomX, randomY).getLocation());
                    if (distance - lastMilestone >= 100 &&
                        this.map.getMapTile(randomX, randomY).getTileType() == TileType.PASSABLE) {
                ...
                case 3:
                    Cosmo cosmo =
                        new Cosmo(4, this.map.getMapTile(randomX, randomY).getLocation());
                    if (distance - lastMilestone >= 100 &&
                        this.map.getMapTile(randomX, randomY).getTileType() == TileType.PASSABLE) {
                ...
                case 4:
                    Daffodil daffodil =
                        new Daffodil(4, this.map.getMapTile(randomX, randomY).getLocation());
                    if (distance - lastMilestone >= 100 &&
                        this.map.getMapTile(randomX, randomY).getTileType() == TileType.PASSABLE) {
                ...
                case 5:
                    Daisy daisy =
                        new Daisy(4, this.map.getMapTile(randomX, randomY).getLocation());
                    if (distance - lastMilestone >= 100 &&
                        this.map.getMapTile(randomX, randomY).getTileType() == TileType.PASSABLE) {
    ...
    // ---- NEW: aim computation prioritizes arrow keys, then movement/facing ----
    private Direction computeAimDirection() {
        // Use arrow keys to aim if held (does NOT affect movement keys WASD)
        if (Keyboard.isKeyDown(Key.LEFT))  return Direction.LEFT;
        if (Keyboard.isKeyDown(Key.RIGHT)) return Direction.RIGHT;
        if (Keyboard.isKeyDown(Key.UP))    return Direction.UP;
        if (Keyboard.isKeyDown(Key.DOWN))  return Direction.DOWN;

        // Otherwise use current walk direction if moving
        if (currentWalkingXDirection != Direction.NONE) return currentWalkingXDirection;
        if (currentWalkingYDirection != Direction.NONE) return currentWalkingYDirection;

        // Then last walking direction
        if (lastWalkingXDirection != Direction.NONE) return lastWalkingXDirection;
        if (lastWalkingYDirection != Direction.NONE) return lastWalkingYDirection;

        // Finally, whatever we’re currently facing, else DOWN
        return (facingDirection != Direction.NONE) ? facingDirection : Direction.DOWN;
    }
    ...
    private void handleAttackInput() {
        ...
        if (justPressed && canAttack) {
            Direction aim = computeAimDirection(); // <<< use aim at fire time

            attacking = true;
            attackStart = System.currentTimeMillis();
            currentAnimationName = "ATTACK_" + aim.name();
    ...
    protected void handlePlayerAnimation() {
        ...
        if (attacking) {
            // (animation name already set in handleAttackInput using aim)
            return;
        }
    ...
    public void draw(GraphicsHandler graphicsHandler) {
        ...
        if (projectileHUD != null) projectileHUD.draw(graphicsHandler);
    ...
    // Existing wrapper uses current aim
    private void shootProjectile() {
        shootProjectile(computeAimDirection());
    }

    // NEW: shoot with explicit aim direction (used by handleAttackInput)
    private void shootProjectile(Direction aim) {
        float projectileX = this.x + 32; // center-ish
        float projectileY = this.y + 32;

        System.out.println("[Bee] Shoot aim=" + aim + " @ (" + projectileX + "," + projectileY + ")");

        BeeProjectile projectile = new BeeProjectile(projectileX, projectileY, aim);
        activeProjectiles.add(projectile);

        System.out.println("[Bee] Projectile created. Total: " + activeProjectiles.size());
    }
    ...
    public void drawProjectiles(GraphicsHandler graphicsHandler) {
        if (map == null || map.getCamera() == null) return;

        float cameraX = map.getCamera().getX();
        float cameraY = map.getCamera().getY();

        for (BeeProjectile projectile : activeProjectiles) {
            projectile.draw(graphicsHandler, cameraX, cameraY);
        }
    }
